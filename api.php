...
if ($action === 'compose') {
    ...
    $sigOptions = '<option value="">No signature</option>';
    foreach ($signatures as $sig) {
        $shortcut = $sig['shortcut'] ? ' (' . $sig['shortcut'] . ')' : '';
        $sigOptions .= '<option value="' . htmlspecialchars($sig['content']) . '">' . htmlspecialchars($sig['name']) . $shortcut . '</option>';
    }

    $sigHint = 'Select a signature or type ';
    if (!empty($signatures)) {
        $shortcuts = array_filter(array_column($signatures, 'shortcut'));
        if (!empty($shortcuts)) {
            $sigHint .= 'shortcuts: ' . implode(', ', $shortcuts);
        } else {
            $sigHint .= 'a / for signature menu';
        }
    } else {
        $sigHint .= 'none configured - add in Settings';
    }
    ...
    <form class="compose-form" id="composeForm">
        ...
        <div class="form-group">
            ...
            <small style="color:var(--text-muted);display:block;margin-top:4px;">' . $sigHint . '</small>
        </div>
