                ...
                foreach ($signatures as $sig): ?>
                    <div class="signature-item" style="margin-bottom:16px;padding:12px;background:var(--bg-main);border:1px solid var(--border);">
                        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
                            ...
                            <button type="button" class="btn-icon" onclick="deleteSignature(<?php echo $i; ?>)" title="Delete"><i class="fa-sharp-duotone fa-thin fa-trash"></i></button>
                        </div>
...
async function deleteSignature(index) {
    ...
    try {
        const response = await fetch("../api.php?action=deleteSignature&index=" + index);
        const result = await response.json();
